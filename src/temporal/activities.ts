...
async function runAgentActivity(
  ...
  try {
    ...
    const isRouterMode = !!(process.env.ANTHROPIC_BASE_URL || process.env.ROUTER_DEFAULT);
    const hasAnthropicCreds =
      !!process.env.ANTHROPIC_API_KEY || !!process.env.CLAUDE_CODE_OAUTH_TOKEN;
    const disableSpendingCapCheck = process.env.SHANNON_DISABLE_SPENDING_CAP_CHECK === 'true';
    if (
      !disableSpendingCapCheck &&
      !isRouterMode &&
      hasAnthropicCreds &&
      result.success &&
      (result.turns ?? 0) <= 2 &&
      ...
      const looksLikeBillingError = /\b(spending\s+cap|budget|quota|billing|rate\s*limit|usage\s*limit|insufficient\s+funds)\b/i.test(resultText);
