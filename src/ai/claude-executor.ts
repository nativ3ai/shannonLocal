...
export async function validateAgentOutput(
  ...
  try {
    ...
    let validationResult = await validator(sourceDir);

    // Fallback: persist model output for agents that require a single deliverable file.
    const fallbackDeliverables: Record<string, string> = {
      'pre-recon': 'deliverables/pre_recon_deliverable.md',
      recon: 'deliverables/recon_deliverable.md',
      report: 'deliverables/comprehensive_security_assessment_report.md',
    };

    const fallbackPath = agentName ? fallbackDeliverables[agentName] : undefined;
    if (!validationResult && fallbackPath && result.result) {
      try {
        const absPath = path.join(sourceDir, fallbackPath);
        await fs.ensureDir(path.dirname(absPath));
        if (!(await fs.pathExists(absPath))) {
          await fs.writeFile(absPath, String(result.result), 'utf8');
        }
        validationResult = await validator(sourceDir);
      } catch (fallbackError) {
        const fallbackMsg =
          fallbackError instanceof Error ? fallbackError.message : String(fallbackError);
        console.log(chalk.yellow(`    Fallback save failed: ${fallbackMsg}`));
      }
    }

    // Fallback: ensure vuln analysis deliverable + queue exist (empty queue if needed).
    const vulnFallbacks: Record<string, { deliverable: string; queue: string }> = {
      'injection-vuln': {
        deliverable: 'deliverables/injection_analysis_deliverable.md',
        queue: 'deliverables/injection_exploitation_queue.json',
      },
      'xss-vuln': {
        deliverable: 'deliverables/xss_analysis_deliverable.md',
        queue: 'deliverables/xss_exploitation_queue.json',
      },
      'auth-vuln': {
        deliverable: 'deliverables/auth_analysis_deliverable.md',
        queue: 'deliverables/auth_exploitation_queue.json',
      },
      'ssrf-vuln': {
        deliverable: 'deliverables/ssrf_analysis_deliverable.md',
        queue: 'deliverables/ssrf_exploitation_queue.json',
      },
      'authz-vuln': {
        deliverable: 'deliverables/authz_analysis_deliverable.md',
        queue: 'deliverables/authz_exploitation_queue.json',
      },
    };

    const vulnFallback = agentName ? vulnFallbacks[agentName] : undefined;
    if (!validationResult && vulnFallback) {
      try {
        const deliverablePath = path.join(sourceDir, vulnFallback.deliverable);
        const queuePath = path.join(sourceDir, vulnFallback.queue);
        await fs.ensureDir(path.dirname(deliverablePath));

        if (result.result && !(await fs.pathExists(deliverablePath))) {
          await fs.writeFile(deliverablePath, String(result.result), 'utf8');
        }

        if (!(await fs.pathExists(queuePath))) {
          const emptyQueue = { vulnerabilities: [] };
          await fs.writeFile(queuePath, JSON.stringify(emptyQueue, null, 2), 'utf8');
        }
        ...
        const fallbackMsg =
          fallbackError instanceof Error ? fallbackError.message : String(fallbackError);
        console.log(chalk.yellow(`    Vuln fallback save failed: ${fallbackMsg}`));
      }
    }
